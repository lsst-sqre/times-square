...
- Improvements to the `GET /v1/pages/{page}/html/events` SSE stream:

  - The `GET /v1/pages/{page}/html/events` SSE stream no longer restates unchanged state once a second. A subscriber now receives one initial event reporting the page instance's current execution and rendering state (with null fields when nothing has happened yet), and after that an event only when that state changes. Each terminal execution failure is reported once, and the server still never closes the stream, so a later re-execution — including one that fails the same way — is reported without resubscribing. Keep-alive is left to the SSE comment pings sent every 15 seconds.

  - The events stream now renders and caches a page instance's HTML when it observes a successful execution, exactly as the HTML endpoint does. Subscribers therefore receive the `html_hash` and `html_url` event as soon as the notebook finishes, without any client requesting the HTML endpoint, and the completed job record is cleared so the stream's idle backoff engages afterwards.
